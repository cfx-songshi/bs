...
- **主要结论一：薄板模型在冲击点的局部量不可信。** 同为0.5 ms窗口，impact_v1给峰值接触力162.51 N、冲击点位移112.2 µm；3D刚球模型给32.0 N、310 µm，即**薄板力高估5.1倍、位移低估2.8倍**。两方向自洽（3D局部柔度更大→球被接住更软→力更小、位移更大）。原因是Rayleigh–Ritz截断把载荷点奇异性正则化。
- **主要结论二：传感器方向依赖已被量化。** 距冲击点73 mm的传感器，沿冲击方向投影196 µε、沿铺设轴投影55 µε，**差3.6倍**；四个象限内比值为1.29/3.26/3.56/4.31。原因：该处主应变方向φ≈42°与冲击方向38°重合，远场以径向变形为主。故P2取"沿冲击方向应变"有物理依据，不是随意选择。
...
- 交叉验证：子模型挠度311.0 µm vs 第二级300.2 µm（3.6%）；另用预设赫兹压力分布表述得302 µm（与310 µm差2.6%）。三套不同网格与接触处理落在同一范围。
- **未解决**：①局部模型能量口径只是近似（力按赫兹分布摊到多节点、间隙取角点，二者非共轭），改用共轭的贴片加权间隙会更差（力401 N、漂移61倍），已试过并回退，**不要再试**；②远场h_max=4 mm未单独做收敛，0.5 ms时全局响应已发展；③两级拆分仅依Saint-Venant在传感器距离成立，接触点附近不能由第二级读取；④无损伤/阻尼/胶层/PZT/压电耦合，**绝不可标为电压**。
