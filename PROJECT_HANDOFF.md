...
- **结果：x方向通过，y方向不通过。** 沿纤维：FE 1306.4（nz=4）/1295.8（nz=8）vs解析1286.4，差1.55%/0.72%。垂直纤维：FE 798.0/798.4 vs"解析"896.1，差10.9%。
- ⚠️ **但该偏差已定位在解析侧，不在FE侧**：①y方向的"解析A0"相速度在85–115 kHz**恒为896.1、完全不随频率变化**，而A0是色散模态，不可能恒定，故该根为假；②直接扫描边界矩阵，c<1480 m/s区间**判别式为负、矩阵无法构造**，即该区间无实解，有效局部极小只有S0(2290)与已排除的c_T(1480)；③根因是横向约束取了`εxx=0`（平面应变），自由板沿y传播应为`σxx=0`（平面应力）。对[0]8该差别在x方向可忽略（ν21=0.017），在y方向却导致解集为空（ν12=0.27）。
- **独立第三方参考**：Kirchhoff薄板给出A0相速度845 m/s（`sqrt(ω)·(D22/(ρh))^(1/4)`，D22=3.49 N·m），因忽略剪切柔度与转动惯量而是**上界**。FE的791–798低于上界6.4%（方向正确），我的解析896高于上界（不可能成立）。
- **已排除的原因**：面内加密(1.25→1.0 mm)10.94%→12.66%、厚度加密(nz4→8)10.94%→10.90%、采样加密(stride2→1)10.94%→10.94%，三者都不改变偏差，旁证其属解析侧。
- 记录一个易误读点：180 mm小板在y方向给1977 m/s的荒谬值，原因是测量时间窗内已含边界反射，不是物理结果。
- **下一步**：把`dispersion.py`的横向约束改为平面应力约化（并重估x方向），再重新验证y方向；否则点源只能算验证了一半。
