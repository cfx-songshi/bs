...
Status: NOT RUN IN ABAQUS. Abaqus is not installed on the machine where this was
written, so the file has never been submitted to the solver. The keyword blocks and
the formatting follow the Abaqus input reference, and the model definition is
deliberately kept to widely used keywords, but treat the first submission as a
syntax check rather than as a validated run. Section 5 of README.md lists what to
look at first.
...
---------------------------
* C3D8R, reduced integration. Full-integration C3D8 locks in bending, which is
  exactly the deformation A0 is made of. Reduced integration carries a hourglass
  risk, which Abaqus controls by default; do not switch that off.
* No orientation block. The laminate is unidirectional with the fibres along the
  global x axis, so the default material directions already coincide with the fibre
  frame. If the layup is ever rotated, an *Orientation must be added, otherwise the
  stiffness stays aligned with the mesh rather than with the fibres.
...
-----
    ...
    abaqus job=plate input=plate.inp cpus=4        # once Abaqus is available
"""
import argparse
import json
import math
...
def emit(path, nx, ny, nz, lx, ly, thickness=THICKNESS, x_src=0.1, mode='line',
    ...
    out = ['*Heading',
           ...
           '** Generated by make_abaqus_inp.py; NOT verified in Abaqus (see README).',
