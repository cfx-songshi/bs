...
### 4c. 点源 400×400×4：应用模式也对照过（2026-09-20）

前面的对照都用线源（验证模式）。用户指定把**应用模式**也做一遍：单节点源在 500×500 mm 板中央，圆波前、四条边都参与反射。两侧同网格 640000 单元 / 804005 节点，都是 `C3D8`（4b 的推荐）。

```powershell
$r = 'D:\abaqus_runs\point400'
& $py make_abaqus_inp.py --nx 400 --ny 400 --nz 4 --lx 0.5 --ly 0.5 --mode point --element C3D8 --x-src 0.25 --out "$r\ugw_point_400x400x4.inp"
& $py solve_uvg_3d.py --mode point --nx 400 --ny 400 --nz 4 --lx 0.5 --ly 0.5 --x-src 0.25 --dt-scale 1.3287 --frames 2d --out out_point_400
```

- Abaqus：1886 增量、稳定步长 **1.19750e-07 s**、odb 10.5 GB。自研：`--dt-scale 1.3287` → dt **1.196953e-07**，两者差 **0.046%**；1838 步、能量漂移 6.4e-14。（该网格 dt_upper_bound = 1.2876e-07，用 0.93 倍是安全的，漂移证实了这一点。）
- ⚠️ **稳定步长与线源那轮不同**：线源 500×4×4 是 9.554e-08，点源 400×400×4 是 1.198e-07（dx 从 1.0 变 1.25 mm）。所以"dz 不变、步长就不变"这个推断是错的，每换网格都要重新读 `.sta` 再对齐自研的 `--dt-scale`。

| 量（两个接收点各 0.18 / 0.32 m） | 自研 | Abaqus C3D8 | 比 |
|---|---|---|---|
| 峰值位移 | 32.7788 nm | 33.9002 nm | **1.034** |
| RMS | 9.6414 nm | 9.7426 nm | **1.010** |
| L2 / 最佳时移 | — | — | **0.3209 / 0.513 µs** |
| 沙漏 / ETOTAL（占峰值 ALLIE） | — | 0 / 0.038% | — |

两个接收点关于中心对称（都在 x 轴上、距源 70 mm），所以两侧的读数**逐位相同**——这同时是一个免费的自检：脚本第一版在收敛段误用了线源窗口（把远端接收点的窗口取成 110–220 µs，跨过了 188 µs 的边缘回波），正是这个对称性把 bug 暴露出来，现已按算例取窗口。

**点源自己也有独立判据**（`check_point_source.py`，用面内各向异性）：

| 方向 | c_p FE | 解析 | 差 |
|---|---|---|---|
| 沿纤维 x | 1306.6 | 1286.4 | **1.57%** |
| 垂直纤维 y | 797.4 | 751.7 | **6.09%** |
| 各向异性比 | 1.639 | 1.711 | **4.26%** |

这与第 3b 节在同样 dx = 1.25 mm 下的旧值（1306.4 / 798.0 / 1.637）一致——说明**离散误差取决于 dx/λ，与板长 250 还是 500 mm 无关**。

⚠️ 顺带记录一个为了跑通 400×400×4 而做的改动：自研装配原来同时保留分批三元组列表**和**它们的 `np.concatenate` 副本，640k 单元时峰值约 20 GB，超过本机 17 GB 空闲内存（`build()` 里的 `rows/cols/vals`）。改为逐个拼接并立即释放分批列表、索引用 int32 后，装配峰值降到约 9 GB、耗时 14.8 s。**回归检查**：500×4×4 用新装配重跑，与旧结果的 `signal`、`frames` **逐位相同**（最大差 0.0），dt、步数、能量漂移也一致，即纯内存改动、数值不变。
...
需要提醒的是第三行：`200×200` 在 500 mm 板上给出 dx = 2.5 mm，只有 5 个单元/A0 波长，**不足以做定量结论**。若要全尺寸点源结果，应取 `--nx 400`（dx = 1.25 mm，10 单元/波长）——**2026-09-20 已按此跑通并与 Abaqus 对照，见第 4c 节**。
...
4. **点源已验证**（见 3b）：沿纤维 1.55%，垂直纤维 3.29%（网格加密后），各向异性比 1.69%。2026-09-20 又在 400×400×4（500 mm 板，dx 1.25 mm）上与 Abaqus C3D8 对照过：RMS 差 **1.1%**、峰值差 3.4%、L2 0.32，各向异性比与解析差 4.26%，见第 4c 节。
5. S0 在本模型的法向激励下依然不可见（与二维同样的问题），因此第 3 节的验证只覆盖 A0。
6. 3D 侧的网格收敛扫描只有线源两档（500×4×4 与 1000×4×8，见 4b 与「波形差异随网格加密收敛」），**点源侧没有做网格收敛**（4c 只是单档对照；点源的收敛性结论目前来自 3b 的 dx 对比）。
...
`out_*/` 为求解输出（`*.npz` 不入库）；特大 `.inp` 亦不入库，点源那套（deck 88 MB、odb 10.5 GB）就留在仓库外的 `D:\abaqus_runs\point400`，详见 [.gitignore](.gitignore)。指标 JSON 与从 odb 抽取的时程 JSON 入库。
