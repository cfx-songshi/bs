...
- **C3D8R（减缩积分，本目录原默认）：在本问题的这一网格上不可用。** 沙漏能占 17–26%，RMS 低 **17%**、峰值低 15–20%，能量进了沙漏模式。要么改用 C3D8，要么另做沙漏控制的收敛研究。下文「建模选择」表里原来那句"C3D8R 配默认沙漏控制即可"已被这一实测推翻。**加密一倍也救不回来**——见「波形差异随网格加密收敛」小节末尾的 C3D8R 加密实验。
...
#### C3D8R 加密一倍：沙漏不降反升（2026-09-20）

既然 4b 判定减缩积分不可用，"加密就会好"是唯一还没试过的退路，所以把 C3D8R 也在 1000×4×8 上跑了一遍（4630 增量、稳定步长 4.77377e-08 s、墙钟 7 秒；与 C3D8 的 4.77432e-08 s 同步长，自研参考仍用同一个 4.77431e-08 s）。
...
**结论：加密一倍改变不了 C3D8R 的可用性。** dx 从 1.0 mm 减到 0.5 mm，沙漏占比 17.26% → 17.28%（毫厘不变，最差采样点反而从 26.4% 升到 31.2%），幅值仍低 9–15%，相速度仍偏 0.81%（C3D8 是 0.09%）。C3D8R 在 1000 网格上与 C3D8 仍差 L2 0.32 / 0.37。**"加密就会好"这条退路已被否定**，本问题继续用 `--element C3D8`；要救减缩积分只能另做沙漏控制（`*Hourglass Control` 增强型等）的研究，未做。

沙漏占比为何在加密后几乎不变，只能作为**观察**记录：`*Bulk Viscosity` 与单元类型都未动，唯一变的是单元尺寸，而占比没有随之下降，说明它不是简单的"网格不够密"。**未做进一步归因**。
...
| 选择 | 理由 |
|---|---|
| 单元类型：见 4b | 原选 `C3D8R`，理由是"全积分 `C3D8` 在弯曲下剪切锁定，而 A0 正是弯曲变形"。实测**推翻了**这个选择：同一网格下 C3D8R 的沙漏能占 ALLIE 的 17–26%、RMS 比自研低 17%，而 C3D8 的 RMS 差 3% 以内、相速度差 0.16%。**加密一倍也没用**（沙漏占比 17.26%→17.28%，见「波形差异随网格加密收敛」小节），故本问题取 `C3D8`；要救减缩积分只剩改沙漏控制这一条路，未做 |
